...
SubShader {
    ...
    CGINCLUDE
        ...
        v2f vert (appdata_t v)
        {
            ...
            // Pass world-space Y so fragments with Y <= 0 can be prioritized
    ...
    // Top-by-Y pass: draw fragments whose world-space Y <= 0 on top (ignore depth)
    Pass {
        Name "TopByY"
        ZWrite Off
        ZTest Always
        ...
        CGPROGRAM
            #pragma vertex vert
            #pragma fragment fragTopByY
            #pragma target 2.0
            #pragma multi_compile_fog
            #include "UnityCG.cginc"

            fixed4 fragTopByY (v2f i) : SV_Target
            {
                fixed4 col = tex2D(_MainTex, i.texcoord);
                // ignore fully transparent texels
                clip(col.a - 0.001);
                // only draw when world-space Y <= 0 (allow small epsilon)
                clip(-i.worldY + 0.0001);
